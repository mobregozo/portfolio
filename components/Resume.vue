<template>
  <section id="wrapper">
    <div class="container-fluid">
      <section
        id="about"
        class="md:h-screen sm:min-h-screen px-10 flex pt-20 md:pt-0"
      >
        <div class="self-center flex-1 ">
          <h1 class="font-bold text-gray-800 text-6xl leading-none">
            MANUEL
            <span class="font-bold text-secondary-400">OBREGOZO</span>
          </h1>
          <h3 class="font-bold text-gray-600 mb-2 text-2xl">
            FRONT END DEVELOPER ·
            <a class="font-bold text-primary-500" href="mailto:email">
              OBREGOZO.MANUEL@GMAIL.COM
            </a>
          </h3>
          <div class="font-bold mb-3 text-gray-500">
            <i class="fas fa-map-marker-alt"></i> AMSTERDAM, NETHERLANDS
          </div>
          <p class="mb-2 mt-8 text-gray-600 font-semibold">
            I am an enthusiastic Front End Developer from a little town called
            Necochea from Argentina. Passionate about software development and
            developer communities.
          </p>
          <p class="mb-2 text-gray-600 font-semibold">
            Currently, I am centered on the web, desktop, and hybrid mobile
            applications, using different JavaScript related Frameworks. I have
            worked as a developer for more than 9 years, in different kind of
            business domains, across a variety of platforms. I executed and
            contributed to full-stack development projects from design to
            testing and up to production deployment, with an emphasis on front
            end features.
          </p>
          <p class="text-gray-600 font-semibold">
            I consider myself a real team player who understands the importance
            of teamwork, proactivity, collaboration, continuous feedback, and
            open communication.
          </p>
          <div class="mt-10">
            <a
              target="blank"
              class="mr-4 text-gray-700 hover:text-gray-400"
              href="https://www.linkedin.com/"
            >
              <span class="fa-stack fa-lg">
                <i class="fas fa-circle fa-stack-2x"></i>
                <i class="fab fa-linkedin-in fa-stack-1x fa-inverse"></i>
              </span>
            </a>
            <a
              target="blank"
              class="mr-4 text-gray-700 hover:text-gray-400"
              href="https://github.com/"
            >
              <span class="fa-stack fa-lg">
                <i class="fas fa-circle fa-stack-2x"></i>
                <i class="fab fa-github-alt fa-stack-1x fa-inverse"></i>
              </span>
            </a>
          </div>
        </div>
      </section>
      <section
        id="experience"
        class="md:h-screen sm:min-h-screen px-10 flex pt-20 md:pt-0"
      >
        <div class="my-auto">
          <h1 class="mb-5 text-6xl font-bold leading-none">Experience</h1>
          <div class="flex flex-col md:flex-row mb-5 py-10">
            <div class="mr-auto">
              <h3 class="m-0 font-bold text-gray-700 text-3xl leading-none">
                ROLE
              </h3>
              <div class="mb-3 font-bold text-gray-600">COMPANY</div>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta
                alias pariatur eaque ad deleniti obcaecati nemo aliquam odit
                perspiciatis sint.Nam sit neque fugiat incidunt voluptatem
                repellat omnis accusantium voluptas!
              </p>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta
                alias pariatur eaque ad deleniti obcaecati nemo aliquam odit
                perspiciatis sint.Nam sit neque fugiat incidunt voluptatem
                repellat omnis accusantium voluptas!
              </p>
            </div>
            <div class="resume-date text-md-right">
              <span class="text-primary-500 font-bold">NOVEMBER </span>
            </div>
          </div>
          <div
            class="flex flex-col md:flex-row mb-5 border-solid border-t-2 border-gray-200 py-10"
          >
            <div class="mr-auto">
              <h3 class="m-0 font-bold text-gray-700 text-3xl leading-none">
                ROLE
              </h3>
              <div class="mb-3 font-bold text-gray-600">COMPANY</div>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta
                alias pariatur eaque ad deleniti obcaecati nemo aliquam odit
                perspiciatis sint.Nam sit neque fugiat incidunt voluptatem
                repellat omnis accusantium voluptas!
              </p>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Soluta
                alias pariatur eaque ad deleniti obcaecati nemo aliquam odit
                perspiciatis sint.Nam sit neque fugiat incidunt voluptatem
                repellat omnis accusantium voluptas!
              </p>
            </div>
            <div class="resume-date text-md-right">
              <span class="text-primary-500 font-bold">NOVEMBER </span>
            </div>
          </div>
        </div>
      </section>

      <section
        id="skills"
        class="md:h-screen sm:min-h-screen resume p-3 p-lg-5 d-flex flex-column"
      >
        <div class="my-auto">
          <h2 class="mb-5">Skills</h2>

          <div class="subheading mb-3">Programming Languages</div>
          <div class="subheading mb-3">Workflow</div>
        </div>
      </section>

      <section
        id="interests"
        class="md:h-screen sm:min-h-screen resume p-3 p-lg-5 d-flex flex-column"
      >
        <div class="my-auto">
          <h2 class="mb-5">Interests / Personal Life</h2>
          <p id="banana">Description.</p>
        </div>
      </section>
    </div>
  </section>
</template>
